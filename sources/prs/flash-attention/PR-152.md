---
id: pr-flash-attention-152
type: source-pr
repo: ROCm/flash-attention
pr: 152
title: Fix Device Segfault
author: micmelesse
date: '2025-07-14'
url: https://github.com/ROCm/flash-attention/pull/152
source_category: upstream-code
architectures:
- cdna2
- cdna3
- cdna4
tags:
- rocm
- flash-attention
- rocm-kernel
kernel_types:
- attention
languages:
- triton-rocm
captured_at: '2026-06-12'
status: merged
inclusion_reason: may contain relevant infrastructure changes
confidence: source-reported
description: If tensors are on a specific device such device 7 and triton launches
  a kernel on the default device such as device 0. We get a segfault. I add asserts
  to catch
---

# Fix Device Segfault

Merged PR #152 in [ROCm/flash-attention](https://github.com/ROCm/flash-attention/pull/152).

**Author:** micmelesse
**Merged:** 2025-07-14

## Description

> If tensors are on a specific device such device 7 and triton launches a kernel on the default device such as device 0. We get a segfault. I add asserts to catch this bug. I also add a bunch of other asserts to insure we are working well formed inputs.

See the PR for full details including code changes and review discussion.

## References

- [PR #152](https://github.com/ROCm/flash-attention/pull/152)
